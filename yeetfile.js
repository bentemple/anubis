...
[
    ...
    [deb, rpm, tarball].forEach(method => method.build({
        ...
        build: ({ bin, etc, systemd, doc }) => {
            $`go build -o ${bin}/anubis -ldflags '-s -w -extldflags "-static" -X "github.com/bentemple/anubis.Version=${git.tag()}"' ./cmd/anubis`;
            $`go build -o ${bin}/anubis-robots2policy -ldflags '-s -w -extldflags "-static" -X "github.com/bentemple/anubis.Version=${git.tag()}"' ./cmd/robots2policy`;
